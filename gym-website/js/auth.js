...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', (e) => {
      ...
      if (missing) {
        if (formError) formError.textContent = `Заполните поле "${fieldLabels[missing] || missing}"`;
        return;
      }
  ...
  function renderUserInfo() {
    ...
    userInfo.innerHTML = `
      ...
      ${u.middleName ? `<p><b>Отчество:</b> ${escapeHtml(u.middleName)}</p>` : ''}
  ...
  function updateHeader() {
    ...
    if (raw) {
      ...
      if (avatar) {
        ...
        avatar.textContent = (u.firstName && u.firstName[0]) ? u.firstName[0].toUpperCase() : 'U';
      }
    } else {
